...
# nQuakesv Clan Arena addon installer bash script (for Linux)
# by Empezar
...
echo
echo "Welcome to the nQuakesv CA installer"
echo "===================================="
echo
...
# Select a hostname
defaulthostname="Clan Arena Server"
...
echo "From what mirror would you like to download the CA addon?"
...
# Download TF addon
echo "=== Downloading ==="
wget --inet4-only -O sv-ca.zip $mirror/sv-ca.zip

# Terminate installation if not all packages were downloaded
if [ -s "sv-ca.zip" ]
then
        if [ "$(du sv-ca.zip | cut -f1)" \> "0" ]
        then
                echo foo >> /dev/null
        else
                echo "Error: The CA addon failed to download. Better luck next time. Exiting."
                cd ..
		rm -rf tmp
                exit
        fi
...
unzip -qqo sv-ca.zip 2> /dev/null;echo "done"
...
echo "#!/bin/sh" >> ../run/ca1.sh
echo "while true ; do" >> ../run/ca1.sh
echo "  ./mvdsv -port $port -game cace +exec port1.cfg" >> ../run/ca1.sh
echo "done" >> ../run/ca1.sh
echo "#end of script" >> ../run/ca1.sh
chmod 755 ../run/ca1.sh
...
echo "./run/ca1.sh > /dev/null &" >> ../start_servers.sh
...
echo "pid=\`ps ax | grep -v grep | grep \"/bin/sh ./run/ca1.sh\" | awk '{print \$1}'\`" >> ../stop_servers.sh
...
cd ../cace/
echo $hostname >> ~/.nquakesv/hostname_ca
