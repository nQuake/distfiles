...
# nQuakesv FFA addon installer bash script (for Linux)
# by Empezar
...
echo
echo "Welcome to the nQuakesv FFA installer"
echo "====================================="
echo
...
echo "From what mirror would you like to download the FFA addon?"
...
# Download TF addon
echo "=== Downloading ==="
wget --inet4-only -O sv-ffa.zip $mirror/sv-ffa.zip

# Terminate installation if not all packages were downloaded
if [ -s "sv-ffa.zip" ]
then
        if [ "$(du sv-ffa.zip | cut -f1)" \> "0" ]
        then
                echo foo >> /dev/null
        else
                echo "Error: The FFA addon failed to download. Better luck next time. Exiting."
                cd ..
                rm -rf tmp
                exit
        fi
...
unzip -qqo sv-ffa.zip 2> /dev/null;echo "done"
...
echo "#!/bin/sh" >> ../run/ffa1.sh
echo "while true ; do" >> ../run/ffa1.sh
echo "  ./mvdsv -port $port -game ffa +exec port1.cfg" >> ../run/ffa1.sh
echo "done" >> ../run/ffa1.sh
echo "#end of script" >> ../run/ffa1.sh
chmod 755 ../run/ffa1.sh
...
echo "./run/ffa1.sh > /dev/null &" >> ../start_servers.sh
...
echo "pid=\`ps ax | grep -v grep | grep \"/bin/sh ./run/ffa1.sh\" | awk '{print \$1}'\`" >> ../stop_servers.sh
...
cd ../ffa/
echo $hostname >> ~/.nquakesv/hostname_ffa
...
ln -s ../ktx/qwprogs.so qwprogs.so
